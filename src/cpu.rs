...
/// Load register pair rr with immediate word nn
macro_rules! LDrr_nn {
    ($cpu:ident, $r1:ident, $r2:ident) => (
        {
...
impl Z80 {
    ...
    /// LD   rr,nn       x1 nn nn  12 ---- rr=nn (rr may be BC,DE,HL or SP)
    /// Load register pair rr with immediate word nn
    fn LDBC_nn(&mut self) { LDrr_nn!(self, b, c); }
    fn LDDE_nn(&mut self) { LDrr_nn!(self, d, e); }
    fn LDHL_nn(&mut self) { LDrr_nn!(self, h, l); }
    fn LDSP_nn(&mut self) {
        let nn = self.read_immediate_word();
        self.regs.sp = nn;
...
// 16-bit loads
#[test]
fn test_the_instruction_set_can_LDrr_nn() {
    let mut cpu = Z80::new();
    cpu.regs.pc = 0xC000;
    cpu.mmu.write_byte(0xC000, 0x05);
    cpu.mmu.write_byte(0xC001, 0x10);
    cpu.LDHL_nn();
    assert_eq!(cpu.regs.hl(), 0x1005); // Little endian
    assert_eq!(cpu.clock.t, 12);
    cpu.mmu.write_byte(0xC002, 0x05);
    cpu.mmu.write_byte(0xC003, 0x10);
    cpu.LDSP_nn();
...
// Same class of opcode, but this one has non-macro logic,
// so we'll double-check
#[test]
fn test_the_instruction_set_can_ADDHLSP() {
    let mut cpu = Z80::new();
    cpu.regs.h = 0x10;
    cpu.regs.l = 0x01;
    cpu.regs.sp = 0x1102;
...
#[test]
fn test_the_instruction_set_can_RLA() {
    let mut cpu = Z80::new();
    cpu.set_flag(CARRY);
    cpu.regs.a = 0b00001001;
    cpu.RLA();
...
#[test]
fn test_the_instruction_set_can_RLr_n() {
    let mut cpu = Z80::new();
    cpu.set_flag(CARRY);
    cpu.regs.b = 0b00001001;
    cpu.RLr_b();
